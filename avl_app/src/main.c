...
int main(int argc, char *argv[])
{
    ...
    const char performance_filename[] = "avl_app_performance.txt";
    ...
    avl_comp_find = 0;                                                              // Reset performance evaluators
    avl_comp_insert = 0;
    ...
    avl_create(&lexicon_words);                                                     // Initialize ADT to store the lexicon's words
    ...
    while(fgets(fileline, MAX_FILELINE + 1, lexicon_handle))                        // Read each of the lexicon's lines until EOF
    {
        ...
        start = clock();
        lexicon_words = avl_insert(lexicon_words, data);                            // Inserts each word bias to the ADT
        end = clock();
        insert_time += (double) (end - start) * 1000 / CLOCKS_PER_SEC;              // Updates total insert time
    }
    fclose(lexicon_handle);                                                         // Close lexicon file

//    avl_print(lexicon_words, Pre_Left);  // FOR TESTING
    ...
    if(output_handle)
    {
        while(fgets(fileline, MAX_FILELINE + 1, sentences_handle))                  // Read each of the sentences until EOF
        {
            ...
            {
                start = clock();
                node = avl_find(lexicon_words, (char *) _strlwr(token));            // Looks for the token's bias
                end = clock();
    ...
    lexicon_words = avl_destroy(lexicon_words);                                     // Free ADT's memory
    ...
    fprintf(performance_handle, "AVL Application Performance:\n\n"                  // Prints results in the performance file
                                ...
                                avl_comp_insert, avl_comp_find, app_time, insert_time, find_time);
